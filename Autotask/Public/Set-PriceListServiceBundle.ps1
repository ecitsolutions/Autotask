...
.OUTPUTS
Nothing or [Autotask.PriceListServiceBundle]. This function optionally returns the updated objects if you use the -PassThru parameter.
.EXAMPLE
Set-PriceListServiceBundle -InputObject $PriceListServiceBundle [-ParameterName] [Parameter value]
Passes one or more [Autotask.PriceListServiceBundle] object(s) as a variable to the function and sets the property by name 'ParameterName' on ALL the objects before they are passed to the Autotask Web Service API and updated.
 .EXAMPLE
$PriceListServiceBundle | Set-PriceListServiceBundle -ParameterName <Parameter value>
Same as the first example, but now the objects are passed to the funtion through the pipeline, not passed as a parameter. The end result is identical.
 .EXAMPLE
Get-PriceListServiceBundle -Id 0 | Set-PriceListServiceBundle -ParameterName <Parameter value>
Gets the instance with Id 0 directly from the Web Services API, modifies a parameter and updates Autotask. This approach works with all valid parameters for the Get function.
 .EXAMPLE
Get-PriceListServiceBundle -Id 0,4,8 | Set-PriceListServiceBundle -ParameterName <Parameter value>
Gets multiple instances by Id, modifies them all and updates Autotask.
 .EXAMPLE
$Result = Get-PriceListServiceBundle -Id 0,4,8 | Set-PriceListServiceBundle -ParameterName <Parameter value> -PassThru
Gets multiple instances by Id, modifies them all, updates Autotask and returns the updated objects.

.LINK
Get-PriceListServiceBundle

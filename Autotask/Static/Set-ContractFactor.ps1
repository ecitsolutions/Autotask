...
.EXAMPLE
Set-ContractFactor -InputObject $ContractFactor [-ParameterName] [Parameter value]
Passes one or more [Autotask.ContractFactor] object(s) as a variable to the function and sets the property by name 'ParameterName' on ALL the objects before they are passed to the Autotask Web Service API and updated.
 .EXAMPLE
$ContractFactor | Set-ContractFactor -ParameterName <Parameter value>
Same as the first example, but now the objects are passed to the funtion through the pipeline, not passed as a parameter. The end result is identical.
 .EXAMPLE
Get-ContractFactor -Id 0 | Set-ContractFactor -ParameterName <Parameter value>
Gets the instance with Id 0 directly from the Web Services API, modifies a parameter and updates Autotask. This approach works with all valid parameters for the Get function.
 .EXAMPLE
Get-ContractFactor -Id 0,4,8 | Set-ContractFactor -ParameterName <Parameter value>
Gets multiple instances by Id, modifies them all and updates Autotask.
 .EXAMPLE
$Result = Get-ContractFactor -Id 0,4,8 | Set-ContractFactor -ParameterName <Parameter value> -PassThru

...
#>
Function New-ContractServiceBundleAdjustment
{
...
If you need very complicated queries you can write a filter directly and pass it using the -Filter parameter. To get the ContractServiceBundleAdjustment with Id number 0 you could write 'New-ContractServiceBundleAdjustment -Id 0' or you could write 'New-ContractServiceBundleAdjustment -Filter {Id -eq 0}.

'New-ContractServiceBundleAdjustment -Id 0,4' could be written as 'New-ContractServiceBundleAdjustment -Filter {id -eq 0 -or id -eq 4}'. For simple queries you can see that using parameters is much easier than the -Filter option. But the -Filter option supports an arbitrary sequence of most operators (-eq, -ne, -gt, -ge, -lt, -le, -and, -or, -beginswith, -endswith, -contains, -like, -notlike, -soundslike, -isnotnull, -isnull, -isthisday). As you can group them using parenthesis '()' you can write arbitrarily complex queries with -Filter. 
...
.EXAMPLE
$Result = New-ContractServiceBundleAdjustment -EffectiveDate [Value]
Creates a new [Autotask.ContractServiceBundleAdjustment] through the Web Services API and returns the new object.
 .EXAMPLE
$Result = Get-ContractServiceBundleAdjustment -Id 124 | New-ContractServiceBundleAdjustment 
Copies [Autotask.ContractServiceBundleAdjustment] by Id 124 to a new object through the Web Services API and returns the new object.
 .EXAMPLE
Get-ContractServiceBundleAdjustment -Id 124 | New-ContractServiceBundleAdjustment | Set-ContractServiceBundleAdjustment -ParameterName <Parameter Value>
Copies [Autotask.ContractServiceBundleAdjustment] by Id 124 to a new object through the Web Services API, passes the new object to the Set-ContractServiceBundleAdjustment to modify the object.
 .EXAMPLE
$Result = Get-ContractServiceBundleAdjustment -Id 124 | New-ContractServiceBundleAdjustment | Set-ContractServiceBundleAdjustment -ParameterName <Parameter Value> -Passthru
Copies [Autotask.ContractServiceBundleAdjustment] by Id 124 to a new object through the Web Services API, passes the new object to the Set-ContractServiceBundleAdjustment to modify the object and returns the new object.

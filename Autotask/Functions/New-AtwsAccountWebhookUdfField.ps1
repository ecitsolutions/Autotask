...
#>
Function New-AtwsAccountWebhookUdfField
{


<#
.SYNOPSIS
This function creates a new AccountWebhookUdfField through the Autotask Web Services API. All required properties are marked as required parameters to assist you on the command line.
.DESCRIPTION
The function supports all properties of an [Autotask.AccountWebhookUdfField] that can be updated through the Web Services API. The function uses PowerShell parameter validation  and supports IntelliSense for selecting picklist values. Any required paramterer is marked as Mandatory in the PowerShell function to assist you on the command line.

If you need very complicated queries you can write a filter directly and pass it using the -Filter parameter. To get the AccountWebhookUdfField with Id number 0 you could write 'New-AtwsAccountWebhookUdfField -Id 0' or you could write 'New-AtwsAccountWebhookUdfField -Filter {Id -eq 0}.

'New-AtwsAccountWebhookUdfField -Id 0,4' could be written as 'New-AtwsAccountWebhookUdfField -Filter {id -eq 0 -or id -eq 4}'. For simple queries you can see that using parameters is much easier than the -Filter option. But the -Filter option supports an arbitrary sequence of most operators (-eq, -ne, -gt, -ge, -lt, -le, -and, -or, -beginswith, -endswith, -contains, -like, -notlike, -soundslike, -isnotnull, -isnull, -isthisday). As you can group them using parenthesis '()' you can write arbitrarily complex queries with -Filter. 
...
#>
  ...
  HelpURI='https://github.com/ecitsolutions/Autotask/blob/master/Docs/New-AtwsAccountWebhookUdfField.md')]
  Param
  (
...
# UDF Field ID
    ...
    [ArgumentCompleter({
      ...
      Get-AtwsPicklistValue -Entity AccountWebhookUdfField -FieldName UdfFieldID -Label -Quoted
    })]
    [ValidateScript({
      $set = (Get-AtwsPicklistValue -Entity AccountWebhookUdfField -FieldName UdfFieldID -Label) + (Get-AtwsPicklistValue -Entity AccountWebhookUdfField -FieldName UdfFieldID -Value)

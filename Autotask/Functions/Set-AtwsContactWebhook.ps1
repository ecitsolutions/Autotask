...
#>
Function Set-AtwsContactWebhook
{


<#
.SYNOPSIS
This function sets parameters on the ContactWebhook specified by the -InputObject parameter or pipeline through the use of the Autotask Web Services API. Any property of the ContactWebhook that is not marked as READ ONLY by Autotask can be speficied with a parameter. You can specify multiple paramters.
.DESCRIPTION
This function one or more objects of type [Autotask.ContactWebhook] as input. You can pipe the objects to the function or pass them using the -InputObject parameter. You specify the property you want to set and the value you want to set it to using parameters. The function modifies all objects and updates the online data through the Autotask Web Services API. The function supports all properties of an [Autotask.ContactWebhook] that can be updated through the Web Services API. The function uses PowerShell parameter validation  and supports IntelliSense for selecting picklist values.

Entities that have fields that refer to the base entity of this CmdLet:


.INPUTS
[Autotask.ContactWebhook[]]. This function takes one or more objects as input. Pipeline is supported.
.OUTPUTS
Nothing or [Autotask.ContactWebhook]. This function optionally returns the updated objects if you use the -PassThru parameter.
.EXAMPLE
Set-AtwsContactWebhook -InputObject $ContactWebhook [-ParameterName] [Parameter value]
Passes one or more [Autotask.ContactWebhook] object(s) as a variable to the function and sets the property by name 'ParameterName' on ALL the objects before they are passed to the Autotask Web Service API and updated.
 .EXAMPLE
$ContactWebhook | Set-AtwsContactWebhook -ParameterName 'Parameter value'
Same as the first example, but now the objects are passed to the funtion through the pipeline, not passed as a parameter. The end result is identical.
 .EXAMPLE
Get-AtwsContactWebhook -Id 0 | Set-AtwsContactWebhook -ParameterName 'Parameter value'
Gets the instance with Id 0 directly from the Web Services API, modifies a parameter and updates Autotask. This approach works with all valid parameters for the Get function.
 .EXAMPLE
Get-AtwsContactWebhook -Id 0,4,8 | Set-AtwsContactWebhook -ParameterName 'Parameter value'
Gets multiple instances by Id, modifies them all and updates Autotask.
 .EXAMPLE
$result = Get-AtwsContactWebhook -Id 0,4,8 | Set-AtwsContactWebhook -ParameterName 'Parameter value' -PassThru
...
#>
  ...
  HelpURI='https://github.com/ecitsolutions/Autotask/blob/master/Docs/Set-AtwsContactWebhook.md')]
  Param
  (

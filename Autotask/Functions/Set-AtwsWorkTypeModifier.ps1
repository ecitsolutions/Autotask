...
#>
Function Set-AtwsWorkTypeModifier
{


<#
.SYNOPSIS
This function sets parameters on the WorkTypeModifier specified by the -InputObject parameter or pipeline through the use of the Autotask Web Services API. Any property of the WorkTypeModifier that is not marked as READ ONLY by Autotask can be speficied with a parameter. You can specify multiple paramters.
.DESCRIPTION
This function one or more objects of type [Autotask.WorkTypeModifier] as input. You can pipe the objects to the function or pass them using the -InputObject parameter. You specify the property you want to set and the value you want to set it to using parameters. The function modifies all objects and updates the online data through the Autotask Web Services API. The function supports all properties of an [Autotask.WorkTypeModifier] that can be updated through the Web Services API. The function uses PowerShell parameter validation  and supports IntelliSense for selecting picklist values.

Entities that have fields that refer to the base entity of this CmdLet:


.INPUTS
[Autotask.WorkTypeModifier[]]. This function takes one or more objects as input. Pipeline is supported.
.OUTPUTS
...
Passes one or more [Autotask.WorkTypeModifier] object(s) as a variable to the function and sets the property by name 'ParameterName' on ALL the objects before they are passed to the Autotask Web Service API and updated.
 .EXAMPLE
$WorkTypeModifier | Set-AtwsWorkTypeModifier -ParameterName 'Parameter value'
Same as the first example, but now the objects are passed to the funtion through the pipeline, not passed as a parameter. The end result is identical.
 .EXAMPLE
Get-AtwsWorkTypeModifier -Id 0 | Set-AtwsWorkTypeModifier -ParameterName 'Parameter value'
Gets the instance with Id 0 directly from the Web Services API, modifies a parameter and updates Autotask. This approach works with all valid parameters for the Get function.
 .EXAMPLE
Get-AtwsWorkTypeModifier -Id 0,4,8 | Set-AtwsWorkTypeModifier -ParameterName 'Parameter value'
Gets multiple instances by Id, modifies them all and updates Autotask.
 .EXAMPLE
$result = Get-AtwsWorkTypeModifier -Id 0,4,8 | Set-AtwsWorkTypeModifier -ParameterName 'Parameter value' -PassThru
...
#>
  ...
  HelpURI='https://github.com/ecitsolutions/Autotask/blob/master/Docs/Set-AtwsWorkTypeModifier.md')]
  Param
  (

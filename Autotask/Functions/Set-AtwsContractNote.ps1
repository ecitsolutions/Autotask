...
#>
Function Set-AtwsContractNote
{


<#
.SYNOPSIS
This function sets parameters on the ContractNote specified by the -InputObject parameter or pipeline through the use of the Autotask Web Services API. Any property of the ContractNote that is not marked as READ ONLY by Autotask can be speficied with a parameter. You can specify multiple paramters.
.DESCRIPTION
This function one or more objects of type [Autotask.ContractNote] as input. You can pipe the objects to the function or pass them using the -InputObject parameter. You specify the property you want to set and the value you want to set it to using parameters. The function modifies all objects and updates the online data through the Autotask Web Services API. The function supports all properties of an [Autotask.ContractNote] that can be updated through the Web Services API. The function uses PowerShell parameter validation  and supports IntelliSense for selecting picklist values.
...
Passes one or more [Autotask.ContractNote] object(s) as a variable to the function and sets the property by name 'ParameterName' on ALL the objects before they are passed to the Autotask Web Service API and updated.
 .EXAMPLE
$ContractNote | Set-AtwsContractNote -ParameterName 'Parameter value'
Same as the first example, but now the objects are passed to the funtion through the pipeline, not passed as a parameter. The end result is identical.
 .EXAMPLE
Get-AtwsContractNote -Id 0 | Set-AtwsContractNote -ParameterName 'Parameter value'
Gets the instance with Id 0 directly from the Web Services API, modifies a parameter and updates Autotask. This approach works with all valid parameters for the Get function.
 .EXAMPLE
Get-AtwsContractNote -Id 0,4,8 | Set-AtwsContractNote -ParameterName 'Parameter value'
Gets multiple instances by Id, modifies them all and updates Autotask.
 .EXAMPLE
$result = Get-AtwsContractNote -Id 0,4,8 | Set-AtwsContractNote -ParameterName 'Parameter value' -PassThru
...
#>
  ...
  HelpURI='https://github.com/ecitsolutions/Autotask/blob/master/Docs/Set-AtwsContractNote.md')]
  Param
  (

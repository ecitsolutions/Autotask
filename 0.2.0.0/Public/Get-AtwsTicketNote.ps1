...
Use Get-help Get-AtwsTicketNote for all possible operators.
      .INPUTS
      Nothing. This function only takes parameters.
      .OUTPUTS
      [Autotask.TicketNote[]]. This function outputs the Autotask.TicketNote that was returned by the API.
      .EXAMPLE
      Get-AtwsTicketNote  -Parameter1 [Parameter1 value] -Parameter2 [Parameter2 Value] -GreaterThan Parameter2
...
by name of "Parameter2" is greater than [Parameter2 Value].
      For parameters, use Get-Help Get-AtwsTicketNote
      .NOTES
      NAME: Get-AtwsTicketNote
  #>
 ...
 ,
        ...
        [Parameter(
         ...
         [ValidateSet('Task Summary','Task Detail','Task Notes','Workflow Rule Note - Task','Duplicate Ticket Note','Outsource Workflow Note','Surveys','Client Portal Note','Taskfire Note','RMM Note','Email2AT')]
 ...
 ,        
        ...
        [ValidateSet('CreatorResourceID','Description','id','LastActivityDate','TicketID','Title')]
        [String[]]
        $NotEquals ,        
  ...
  {     
    ...
    If (-not($Filter))
    {
        $Fields = $Atws.GetFieldInfo('TicketNote')
    ...
    Get-AtwsData -Entity TicketNote -Filter $Filter }   
